...
function createSchedule($conn)
{
    ...
    if ($stmt->execute()) {
        ...
        // Log activity
        logActivity($conn, 'create_schedule', 'schedule', $scheduleId, "Tạo lịch chiếu mới vào ngày {$scheduleDate} cho media ID {$mediaId}");
...
function updateSchedule($conn)
{
    ...
    if ($stmt->execute()) {
        logActivity($conn, 'update_schedule', 'schedule', $id, "Cập nhật lịch chiếu ID {$id}");
...
function deleteSchedule($conn)
{
    ...
    if ($stmt->execute()) {
        logActivity($conn, 'delete_schedule', 'schedule', $id, "Xóa lịch chiếu ID {$id}");
