...
function renderUsers() {
    ...
    tbody.innerHTML = users.map(user => {
        ...
        const roleClass = user.role === 'super_admin' ? 'badge-admin' : 'badge-manager';
        const roleLabel = user.role === 'super_admin' ? 'Super Admin' : 'Content Manager';
        ...
        return `
            <tr>
                ...
                <td><span class="badge ${roleClass}"><i class="fas fa-${user.role === 'super_admin' ? 'shield-alt' : 'user'}"></i> ${roleLabel}</span></td>
                <td><span class="badge ${statusClass}"><i class="fas fa-${user.status === 'active' ? 'check' : 'ban'}"></i> ${statusLabel}</span></td>
                <td>${user.last_login ? formatDate(user.last_login) : '<span style="color: rgba(255,255,255,0.4)">Chưa đăng nhập</span>'}</td>
                <td>${formatDate(user.created_at)}</td>
                <td>
                    <div class="action-buttons">
                        <button class="btn-action edit" onclick="editUser(${user.id})" title="Sửa">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn-action reset" onclick="resetPassword(${user.id}, '${escapeHtml(user.username)}')" title="Đặt lại mật khẩu" ${isCurrentUser ? 'disabled' : ''}>
                            <i class="fas fa-key"></i>
                        </button>
                        <button class="btn-action delete" onclick="confirmDelete(${user.id}, '${escapeHtml(user.full_name || user.username)}')" title="Xóa" ${isCurrentUser ? 'disabled' : ''}>
